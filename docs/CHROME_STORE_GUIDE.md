...
```powershell
Compress-Archive -Path manifest.json, newtab.html, PRIVACY.md, README.md, LICENSE, icons, src -DestinationPath nordlys-v2.2.3.zip -Force
```
...
```powershell
Expand-Archive nordlys-v2.2.3.zip -DestinationPath .\zip-check -Force; Get-ChildItem .\zip-check
```
...
3. Click **Add new item** and upload `nordlys-v2.2.3.zip`.
