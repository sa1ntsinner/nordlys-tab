...
- **A board you arrange.** Arrange mode lets you put folders in the rows you want and shows where one will land before you let go. *Fitted* runs every row edge to edge; *Tidy up* pairs folders of a similar height. Bookmark size, the space between bookmarks and between folders, the board's width and the names under the icons are set in the same place, with the board as the preview and Undo for each step.
- **Icons that look intentional.** Search a curated brand-mark collection by product name, preview the result on the real tile, and save the chosen vector locally. Chrome's own favicon cache, an image URL, a local file, and monograms remain as fallbacks. An icon taken from a web address remembers it, and the last few it used, so you can go back to one. There's a small cropper with pan, zoom, and rotate for images that arrive with extra space.
- **Search that takes commands.** Type `>` for `theme nord`, `sky frost`, `move YouTube to Daily`, `new folder Reading` or `arrange`; the page shows the result before Enter keeps it, with one undo.
...
- **Background atmospheres.** Six authored procedural compositions — Nordlys, Halo, Silk, Frost, Contour and Fjord — share a set of colour moods you can add to, and one switch lights any of them by the time of day where you are, worked out from your time zone with nothing sent anywhere. Halo shows tonight's moon. The sky behind the clock, the greeting and the search box is measured as it moves, and held back just enough to keep that text above WCAG AA. Your own image or looping video is stored locally in IndexedDB, and a flat theme colour stays available. Still mode paints one frame and stops; Reduce Motion gets the same composed picture without the loop.
