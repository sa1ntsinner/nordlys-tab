...
3. **Website icons (only when you choose a remote source):** Opening the icon picker makes no request. The "Website icon" tab reads from Chrome's **local** favicon cache by default — no network at all. A remote provider is contacted only when you press its chip: Google's favicon service (`www.google.com/s2/favicons`), DuckDuckGo's icon service (`icons.duckduckgo.com`), or the site's own `/apple-touch-icon.png`. Only the domain you typed is included in the request, and a failed lookup never falls back to another provider on its own.
